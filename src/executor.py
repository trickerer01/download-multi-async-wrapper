...
async def run_cmd(query: str, dt: str, qn: int, qt: str, qtn: int) -> None:
    ...
    log_file_name = f'{Config.dest_logs_base}log_{suffix}{dt}{qn:{dtqn_fmt()}}_{qt}{qtn:{dtqn_fmt()}}_{exec_time}.log'
    with open(log_file_name, 'at', encoding=UTF8, buffering=1) as log_file:
        ...
        if dt in RUN_FILE_DOWNLOADERS and len(query) > Config.max_cmd_len:
            run_file_name = f'{Config.dest_run_base}run_{suffix}{dt}{qn:{dtqn_fmt()}}_{qt}{qtn:{dtqn_fmt()}}_{exec_time}.conf'
