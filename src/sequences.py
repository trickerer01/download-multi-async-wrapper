    ...
    [base_qs.update({
        k: {
            dt: (f'{Config.python} "{sequences_paths[k][dt]}" '
                 ...
                 f'{f" {rs % irngs[k][dt].first}" if irngs[k][dt].first and page_ids(dt) else ""}'
                 f'{f" {rb % irngs[k][dt].second}" if irngs[k][dt].second and page_ids(dt) else ""}')
...
def form_queries(
    ...
    [queries_final.update({
        k: {
            dt: ([f'{base_qs[k][dt]} {path_args(Config.dest_base, k, ssubs[k][dt][i], not Config.no_date_path)} '
                ...
                ([f'{base_qs[k][dt]} {path_args(Config.dest_base, k, "", not Config.no_date_path)} '
                  f'{" ".join(scomms[k][dt])} '
                  f'-script "'
