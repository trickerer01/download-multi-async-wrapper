...
def validate_runners(queries: Queries) -> None:
    if 'all' in Config.validated_runners:
        return
    ...
    if runner_type_python not in Config.validated_runners:
        ...
        Config.validated_runners.add(runner_type_python)
    ...
    if Config.install:
        for dtr, rpath in queries.sequences_paths_reqs.items():
            ...
            if not rpath or runner_type_install in Config.validated_runners or dtr not in Config.downloaders:
                continue
            ...
            try:
                ...
                Config.validated_runners.add(runner_type_install)
                trace('Done')
            except Exception:
    ...
    if not Config.no_download:
        for cat in queries.sequences_paths:
            for dtd in queries.sequences_paths[cat]:
                ...
                if runner_type_download in Config.validated_runners:
                    continue
                ...
                try:
                    ...
                    Config.validated_runners.add(runner_type_download)
                    trace(f'Found \'{dpath}\'')
                except Exception:
    ...
    if Config.update:
        for dtu, upath in queries.sequences_paths_update.items():
            ...
            if runner_type_update in Config.validated_runners:
                continue
            ...
            try:
                ...
                Config.validated_runners.add(runner_type_update)
                trace(f'Found \'{upath}\'')
            except Exception:
                trace(f'Error: invalid {dtu} updater found at: \'{upath}\'!')
                raise OSError
    Config.validated_runners.add('all')
